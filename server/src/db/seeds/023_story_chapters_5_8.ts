...
export async function seed(knex: Knex): Promise<void> {
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(32),
    phase_order: 2,
    title: "Scan the Wreckage",
    description:
      "Scan 2 wreckage sites to identify the attackers' unknown technology.",
  ...
  await upsertPhase(knex, {
    ...
    phase_order: 1,
    title: "Scan Shadow Sites",
    description: "Scan 4 sectors where shadow energy has concentrated.",
    objective_type: "scan_sectors",
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(36),
    phase_order: 2,
    title: "Survive the Shadow Attack",
    description:
      "Survive an attack from a Shadow Construct guardian drawn by your scans.",
    objective_type: "survive_ambush",
    objectives: JSON.stringify({
      ambushesToSurvive: 1,
      npcNames: ["Shadow Construct", "Void Walker"],
    }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    phase_order: 1,
    title: "Verify Warp Destination",
    description:
      "Scan 3 sectors near the warp gate exit to check for Precursor defenses.",
    objective_type: "scan_sectors",
  ...
  await upsertPhase(knex, {
    ...
    description: "Deliver 20 food to the coalition forward base.",
    objective_type: "deliver_cargo",
  ...
  await upsertPhase(knex, {
    ...
    description:
      "Use the Jump Drive to navigate through 4 waypoint sectors toward the galaxy's center.",
    objective_type: "visit_sector",
    objectives: JSON.stringify({ sectorsToVisit: 4 }),
    target_resolution: JSON.stringify({
      type: "galaxy_center_waypoints",
      params: { count: 4 },
    }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(40),
    phase_order: 2,
    title: "Survive the Sentinel",
    description:
      "Survive an ambush from a Primordium Sentinel at the core threshold.",
    objective_type: "survive_ambush",
    objectives: JSON.stringify({
      ambushesToSurvive: 1,
      npcNames: ["Primordium Sentinel"],
      npcShipType: "corvette",
    }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    phase_order: 1,
    title: "Map the Trap Field",
    description:
      "Scan 4 sectors to map the Precursor trap mechanisms before they claim more victims.",
    objective_type: "scan_sectors",
    objectives: JSON.stringify({ scansRequired: 4 }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(42),
    phase_order: 2,
    title: "Scan the Primordium",
    description:
      "Scan 3 Primordium structures to decode the ancient civilization's name, purpose, and final warning.",
    objective_type: "scan_sectors",
    objectives: JSON.stringify({ scansRequired: 3 }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    phase_order: 1,
    title: "Scout the Staging Area",
    description:
      "Scan 4 sectors for enemy positions and safe approach vectors before the fleet advances.",
    objective_type: "scan_sectors",
    objectives: JSON.stringify({ scansRequired: 4 }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    description:
      "Destroy 5 Primordium sentinels guarding the final corridor to the core.",
    objective_type: "destroy_ship",
    objectives: JSON.stringify({
      shipsToDestroy: 5,
      npcNames: [
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(45),
    phase_order: 2,
    title: "Reach the Core Heart",
    description:
      "Navigate to the centermost sector of the galaxy — the core heart.",
    objective_type: "visit_sector",
    objectives: JSON.stringify({ sectorsToVisit: 1 }),
    target_resolution: JSON.stringify({ type: "galaxy_center" }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    description: "Destroy 8 Primordium warships in the opening assault.",
    objective_type: "destroy_ship",
    objectives: JSON.stringify({
      shipsToDestroy: 8,
      npcNames: [
  ...
  await upsertPhase(knex, {
    ...
    description: "Survive 2 waves of Primordium reinforcements.",
    objective_type: "survive_ambush",
    objectives: JSON.stringify({
      ambushesToSurvive: 2,
      npcNames: ["Primordium Elite", "Primordium Destroyer"],
      npcShipType: "cruiser",
    }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(49),
    phase_order: 2,
    title: "Establish the Trade Route",
    description:
      "Trade 20 units at the route destination to prove the network works.",
    objective_type: "trade_units",
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(51),
    phase_order: 2,
    title: "Scan Precursor Sites",
    description:
      "Scan 3 Precursor sites to confirm guardian systems are dormant and safe for conversion.",
    objective_type: "scan_sectors",
    objectives: JSON.stringify({ scansRequired: 3 }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    template_id: storyMissionId(55),
    phase_order: 2,
    title: "Visit Primordium Archives",
    description:
      "Visit 3 intact Primordium archive sites to activate data downloads.",
    objective_type: "visit_sector",
    objectives: JSON.stringify({ sectorsToVisit: 3 }),
    lore_text:
  ...
  await upsertPhase(knex, {
    ...
    phase_order: 1,
    title: "Supply the Expedition",
    description:
      "Deliver 20 tech to the inter-galactic expedition staging area.",
    objective_type: "deliver_cargo",
    objectives: JSON.stringify({ commodity: "tech", quantity: 20 }),
    lore_text:
